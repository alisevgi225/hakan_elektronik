# SEO Optimizasyon Rehberi - Hakan Elektronik Malatya

## ✅ Yapılan SEO Optimizasyonları

### 1. Meta Tags
- ✅ Title tags (her sayfa için özel)
- ✅ Meta description (her sayfa için özel)
- ✅ Meta keywords (Malatya odaklı)
- ✅ Open Graph tags (Facebook paylaşımları için)
- ✅ Twitter Card tags
- ✅ Canonical URLs

### 2. Schema.org Structured Data
- ✅ LocalBusiness schema (Google'da işletme bilgileri için)
- ✅ Breadcrumb schema (market.html için)
- ✅ OfferCatalog schema (ürün kataloğu için)

### 3. Teknik SEO
- ✅ robots.txt dosyası
- ✅ sitemap.xml dosyası
- ✅ Canonical URLs
- ✅ Mobile-friendly (responsive tasarım)
- ✅ Fast loading (optimize edilmiş)

## 📋 Yapılması Gerekenler

### 1. Google Business Profile (ÖNEMLİ!)
1. Google Business Profile oluşturun: https://business.google.com
2. İşletme bilgilerini ekleyin:
   - Tam adres (Malatya)
   - Telefon: +905459685329
   - Çalışma saatleri
   - Kategori: Elektronik Mağazası / POS Sistemleri
   - Fotoğraflar ekleyin
3. Müşteri yorumları toplayın
4. Düzenli olarak güncelleyin

### 2. Google Search Console
1. https://search.google.com/search-console adresine gidin
2. Sitenizi ekleyin
3. Sitemap.xml'i gönderin: `https://www.hakanelektronik.com/sitemap.xml`
4. robots.txt'i kontrol edin
5. Indexleme durumunu takip edin

### 3. İçerik Optimizasyonu
- Hakkımızda bölümüne "Malatya" kelimesini ekleyin
- Ürün açıklamalarında "Malatya" geçirin
- Blog yazıları ekleyin (örnek: "Malatya'da POS Sistemi Seçimi")

### 4. Backlink Stratejisi
- Yerel dizinlere kayıt olun (Malatya işletme rehberleri)
- Sosyal medya hesapları oluşturun
- Müşteri referansları ekleyin

### 5. Yerel SEO
- Malatya işletme rehberlerine kayıt:
  - Yandex Haritalar
  - Foursquare
  - Yelp
  - Yerel Malatya rehberleri

### 6. Sayfa Hızı Optimizasyonu
- Resimleri optimize edin (WebP formatı)
- CSS ve JS dosyalarını minify edin
- CDN kullanın

### 7. Sosyal Medya
- Facebook sayfası oluşturun
- Instagram hesabı açın
- LinkedIn profil oluşturun
- Düzenli içerik paylaşın

## 🎯 Anahtar Kelimeler (Malatya Odaklı)

### Birincil Anahtar Kelimeler:
- POS Malatya
- Barkod Yazıcı Malatya
- Yazarkasa Malatya
- Para Sayma Makinesi Malatya
- Barkod Okuyucu Malatya
- Terazi Malatya
- POS Cihazı Malatya
- Hakan Elektronik Malatya

### İkincil Anahtar Kelimeler:
- Elektronik Malatya
- POS Sistemi Malatya
- Barkod Sistemi Malatya
- Yazarkasa Fiyatları Malatya
- POS Cihazı Fiyatları Malatya

## 📊 Takip Edilmesi Gereken Metrikler

1. **Google Search Console**
   - Tıklama sayısı (Clicks)
   - Görüntülenme (Impressions)
   - CTR (Click-Through Rate)
   - Ortalama pozisyon

2. **Google Analytics**
   - Organik trafik
   - Sayfa görüntüleme
   - Bounce rate
   - Ortalama oturum süresi

3. **Google Business Profile**
   - Görüntülenme
   - Arama sorguları
   - Yönlendirmeler
   - Yorumlar

## 🔧 Güncelleme Gereken Bilgiler

### index.html ve market.html'de güncellenmesi gerekenler:
1. **Adres bilgisi**: Tam adres ekleyin (Schema.org'da)
2. **Çalışma saatleri**: Gerçek saatleri güncelleyin
3. **E-posta**: info@hakanelektronik.com (doğru mu kontrol edin)
4. **Website URL**: https://www.hakanelektronik.com (domain alındığında güncelleyin)

## 📝 İçerik Önerileri

### Blog Yazıları:
1. "Malatya'da POS Sistemi Seçerken Dikkat Edilmesi Gerekenler"
2. "Barkod Yazıcı Alırken Bilmeniz Gerekenler"
3. "Para Sayma Makinesi Nasıl Seçilir?"
4. "Malatya İşletmeleri İçin POS Çözümleri"
5. "Barkod Sistemi Kurulumu ve Avantajları"

### Sayfa İçerikleri:
- Her kategori sayfasına detaylı açıklama ekleyin
- Ürün sayfalarına daha fazla içerik ekleyin
- FAQ (Sık Sorulan Sorular) bölümü ekleyin

## 🚀 Hızlı Başlangıç Checklist

- [ ] Google Business Profile oluştur
- [ ] Google Search Console'a kayıt ol
- [ ] Sitemap.xml'i Google'a gönder
- [ ] Sosyal medya hesapları oluştur
- [ ] Yerel dizinlere kayıt ol
- [ ] İlk blog yazısını yayınla
- [ ] Müşteri yorumları topla
- [ ] Google Analytics kurulumu yap
- [ ] Adres ve iletişim bilgilerini güncelle
- [ ] Resimleri optimize et

## 📞 Destek

SEO ile ilgili sorularınız için:
- Email: info@hakanelektronik.com
- WhatsApp: +905459685329

---

**Not**: SEO sonuçları 3-6 ay içinde görülmeye başlar. Sabırlı olun ve düzenli olarak içerik ekleyin!


